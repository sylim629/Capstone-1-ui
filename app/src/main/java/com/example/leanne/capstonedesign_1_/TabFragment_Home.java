    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 1; j < topN; i += 11, j++) {
            ...
            for (int k = 0; k < idFavs.size(); k++) {
                // LoggedInUser의 Fav ID와 비교해서 일치하면 isFav = true. 아니면 false
                isFav = idInfo.equals(idFavs.get(k));
            }
            String majorInfo = "전공 : ";
            String dutyInfo = "직무 : ";
            String certifInfo = "자격증 : ";
            String toeicInfo = "토익 :  ";

            idInfo += tokens[i];
            majorInfo += tokens[i + 3];
            dutyInfo += tokens[i + 5];
            certifInfo += tokens[i + 9];
            toeicInfo += tokens[i + 1];
            certifInfo = certifInfo.replace("|", ",");
            mItems.add(new ListViewItem(ranking, idInfo, majorInfo, dutyInfo, certifInfo, toeicInfo, isFav));  // 여기서 isFav도 같이 저장해서 넘김
        }
/*
        // 일단은 컴공 밖에 선택 못 하니까
